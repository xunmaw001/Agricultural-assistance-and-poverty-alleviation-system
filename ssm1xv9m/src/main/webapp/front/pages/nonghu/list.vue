	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
        ...
        <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==0" class="search-form round">
				...
				<input v-model="searchForm.nonghuzhanghao" type="text" placeholder="农户账号" ></input>
			</view>
			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.nonghuxingming" type="text" placeholder="农户姓名" ></input>
			</view>
			<view class="action">
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"#efefef"}'>
			<view @tap="onDetailTap(product)" class="uni-product" :style='{"borderRadius":"8rpx","backgroundColor":"#fff"}' v-for="(product,index) in list" :key="index">
					...
					<text v-if="isAuth('nonghu','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('nonghu','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
				</view>
			</view>
		</view>
		
		
		<button v-if="isAuth('nonghu','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.nonghuzhanghao="";
				this.searchForm.nonghuxingming="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				let res = await this.$api.list(`nonghu`, params);
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('nonghu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				let searchForm = {
					...
					limit: this.mescroll.size
												...
												if(this.searchForm.nonghuzhanghao){
					searchForm['nonghuzhanghao'] = '%' + this.searchForm.nonghuzhanghao + '%'
				}	
																								if(this.searchForm.nonghuxingming){
					searchForm['nonghuxingming'] = '%' + this.searchForm.nonghuxingming + '%'
				}	
																																																																				let res = await this.$api.list(`nonghu`, searchForm);
				// 如果是第一页数据置空
